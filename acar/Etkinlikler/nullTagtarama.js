...
module.exports = {
  ...
  (acar.kayıtRolleri.kayıtsızRolleri).client.guilds.cache.get(sid).members.cache.filter(uye => uye.roles.cache.size === 1).array().forEach((uye, index) => setTimeout(() => { uye.roles.add(acar.kayıtRolleri.kayıtsızRolleri).catch(console.error); uye.setNickname('✦ İsim | Yaş'); }));

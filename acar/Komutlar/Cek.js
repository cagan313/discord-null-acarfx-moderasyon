  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if (message.member.roles.highest.position < uye.roles.highest.position) { 
      const reactionFilter = (reaction, user) => {
        return ['☑️'].includes(reaction.emoji.name) && user.id === uye.id;
      };
     ...
     if (acar.Roller.botCommand.some(rol => message.member.roles.cache.has(rol)) || message.member.hasPermission('ADMINISTRATOR'))  {

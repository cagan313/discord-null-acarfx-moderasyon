...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if((!acar.erkekRolleri && !acar.kadinRolleri) || !acarveri.Roller.istatistikciRolleri) return message.channel.send("Sistemsel hata: Rol bulunamadı veya rol bilgileri girilemedi.").then(sil => sil.delete({timeout: 5000}));
    if(!acarveri.Roller.istatistikciRolleri.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(`Hata: Bu komutu kullanabilmek için yeterli yetkiye sahip değilsin.`).then(sil => sil.delete({timeout: 5000}));
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}isim @xeox/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
    ...
    if(!isim || !yaş) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}isim @xeox/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
  ...
  message.channel.send(embed.setThumbnail(acarveri.embedUfakResim).setDescription(`» İsmi Değişen Üye: ${uye}\n» Güncellenen İsim: \`${BelirlenenIsim}\``).addField(`Bu Kullanıcının Geçmiş İsimleri [${isimdata.length}]` || `0`, isimler, true)).then(sil => sil.delete({timeout: 7000}));
  message.react("☑️")
    }
};

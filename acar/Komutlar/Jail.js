...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}jail @xeox/ID <1s/1m/1h/1d> <sebep>\``).then(sil => sil.delete({timeout: 5000}));
  ...
  if(!sure || !ms(sure) || !reason) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}jail @xeox/ID <1s/1m/1h/1d> <sebep>\``).then(sil => sil.delete({timeout: 5000}));
  ...
  message.channel.send(`${uye} kişisi **${reason}** nedeni ile **${jailzaman}** süresince __cezalıya atıldı__. (Ceza Numarası: #${cezano})`).catch().then(sil => sil.delete({timeout: 10000}));
  message.react("☑️")

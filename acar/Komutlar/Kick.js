...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if (!victim || !reason) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}kick @xeox/ID sebep\``).then(x => x.delete({timeout: 5000}));
    ...
    victim.kick({reason: reason}).then(x => message.react("☑️")).catch();
    let ceza = {
        No: cezano,
        Cezalanan: victim.id,

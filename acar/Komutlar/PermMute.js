...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}permmute @xeox/ID <sebep>\``).then(sil => sil.delete({timeout: 5000}));
    ...
    message.channel.send(`${uye} kişisi **${reason}** nedeni ile __kalıcı olarak__ metin kanallarında __susturuldu__. (Ceza Numarası: #${cezano})`).catch().then(sil => sil.delete({timeout: 10000}));
    message.react("☑️")

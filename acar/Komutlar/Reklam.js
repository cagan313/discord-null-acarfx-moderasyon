...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye || !reason) return message.channel.send(`Hata: Lütfen üye belirleyin veya sebep belirleyin!  __Örn:__  \`${client.sistem.a_Prefix}reklam @xeox/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  ...
  uye.setNickname(`Reklamcıyız Dayı`).catch();
  message.channel.send(`${uye} kişisi **${reason}** nedeni ile süresiz olarak __cezalıya atıldı__. (Ceza Numarası: #${cezano})`).catch().then(sil => sil.delete({timeout: 10000}));
  message.react("☑️")

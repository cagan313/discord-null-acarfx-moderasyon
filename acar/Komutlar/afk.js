...
module.exports = {
    ...
    client.on("message", (message) => {
        ...
        if(message.mentions.users.size >= 1){
          ...
          if(db.has(`${victim.id}.afk`)) {
            ...
            return message.channel.send(`${victim} adlı üyesi ${tarih} **__AFK__** oldu.`).then(x => x.delete({timeout: 10000}));;
          };
        };
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    let sebep = args.join(' ') || `Xeox la film izliyok sal`;
    if(sebep.includes("@here") || sebep.includes("@everyone")) return message.channel.send(`Hata: Lütfen geçerli bir __AFK__ sebebi belirt. ${message.member}`).then(x => x.delete({timeout: 5000}));;
    if (sebep && (await client.afkKoruma(sebep))) return message.channel.send(`Hata: Lütfen geçerli bir __AFK__ sebebi belirt. ${message.member}`).then(x => x.delete({timeout: 5000}));;
    ...
    message.channel.send(`${message.member} adlı üye ${tarih} **__AFK__** moduna girdi!`).then(x => x.delete({timeout: 10000}));
    message.react("☑️")

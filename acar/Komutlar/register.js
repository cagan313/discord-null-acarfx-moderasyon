...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    var rolismi = "Register"
    var rolid = acarveri.Roller.register
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if (!acarveri.Roller.banHammer.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('MANAGE_ROLES') && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`Hata: Bu komutu kullanabilmek için yeterli yetkiye sahip değilsin!`).then(x => x.delete({timeout: 5000}));
